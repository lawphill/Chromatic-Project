...
for i = 1:n(1)
    ...
    *((factorial(nsockets-data(i,7)))/(factorial(data(i,8))*factorial(data(i,9))));
    ...
    pnew = ((data(i,1).^1+X)./(sum(data(i,1:3).^2)+3.*X)).^data(i,7) .* ...
        ((data(i,2).^2+X)./(sum(data(i,1:3).^2)+3.*X)).^data(i,8) .* ...
        ((data(i,3).^2+X)./(sum(data(i,1:3).^2)+3.*X)).^data(i,9) ...
        .* ncombin;
    
    % calculate odds of landing the exact same configuration
    pold = ((data(i,1).^2+X)./(sum(data(i,1:3).^2)+3.*X)).^data(i,4) .* ...
        ((data(i,2).^2+X)./(sum(data(i,1:3).^2)+3.*X)).^data(i,5) .* ...
        ((data(i,3).^2+X)./(sum(data(i,1:3).^2)+3.*X)).^data(i,6);
